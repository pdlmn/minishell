...
/*   Updated: 2024/04/12 20:06:28 by emuminov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		enum e_quotes is_quoted)
{
	...
	if ((is_quoted == NOT_QUOTED && ft_strchr("\"'~$", *input))
		...
		|| (is_quoted != SQUOTED && input[0] == '$')
		|| (is_quoted != SQUOTED && ft_isdigit(input[0]) && last_t && last_t->type == SIGIL)
		|| ((is_quoted == NOT_QUOTED || is_quoted == DQUOTED) && !is_valid_variable_char(*input) && *input != ' '))
